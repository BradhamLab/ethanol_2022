...
if __name__ == '__main__':
    ...
    if snakemake is not None:
        ...
        annos = pd.read_csv(snakemake.input['annos'],
                           index_col=0,
                           delimiter='\t')
        ...
        annos['gname'] = annos.apply(lambda x: get_gene_name(x.name,
                                                             annos,
                                                             names),
                                     axis=1)
        counts['name'] = counts.apply(lambda x: get_gene_name(x.name,
                                                              annos,
                                                              names),
                                      axis=1)
        ...
        out.rename(columns={x:f"sample-{x}" for x in out.columns},
                   inplace=True)
